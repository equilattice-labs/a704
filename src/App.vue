...
const activePool = ref('deedluma / Robin')
...
const pools = [
  {name:'deedluma / Robin', pair:'DPHL / RBH', apy:'42.8%', tvl:'$18.4M', risk:'Low', color:'violet', change:'+12.4%'},
  {name:'Robin / USDC', pair:'RBH / USDC', apy:'28.6%', tvl:'$11.2M', risk:'Low', color:'blue', change:'+8.7%'},
  {name:'deedluma / ETH', pair:'DPHL / ETH', apy:'67.1%', tvl:'$6.8M', risk:'Medium', color:'pink', change:'+24.1%'}
]
  ...
  <div class="app-shell">
    ...
    <aside class="sidebar">
      <div class="brand" @click="navTo('Overview')">
        ...
        <div><strong>DEED<span>LUMA</span></strong><small>LIQUIDITY, IN MOTION</small></div>
      </div>
    ...
    <main class="workspace">
      <header class="topbar"><div class="mobile-brand">DEED<span>LUMA</span></div><div class="crumb">OVERVIEW <span>/</span> LIQUIDITY DESK</div><div class="top-actions"><div class="block-pill"><span class="status-dot"></span> Block #8,421,903</div><button v-if="!connected" class="connect-btn" @click="connectWallet">{{isConnecting ? 'Connecting…' : 'Connect wallet'}} <span>↗</span></button><button v-else class="account-btn" @click="disconnect"><span class="status-dot"></span>{{shortAccount}}</button></div></header>
      ...
      <footer><div class="brand-mini">◈ DEEDLUMA</div><span>© 2025 deedluma Protocol</span><div class="footer-links"><button class="footer-link" @click="openDocs">Docs</button><a href="https://x.com/deedluma_fun" target="_blank" rel="noopener noreferrer">X ↗</a></div></footer>
    </main>
    <div v-if="showDocs" class="modal-backdrop" @click.self="showDocs=false"><div class="modal docs-modal" role="dialog" aria-modal="true" aria-labelledby="docs-title"><button class="modal-close" aria-label="Close protocol brief" @click="showDocs=false">×</button><div class="eyebrow">DEEDLUMA DOCS</div><h2 id="docs-title">Protocol brief</h2><p class="modal-sub">A quick guide to providing liquidity on Robinhood Chain Testnet.</p><div class="docs-section"><h3>How it works</h3><p>deedluma routes swaps through community-owned pools. Liquidity providers deposit one asset, earn a share of trading fees and can withdraw at any time.</p></div><div class="docs-section"><h3>Get started</h3><ol><li>Connect an EVM wallet and switch to Robinhood Chain Testnet.</li><li>Choose a pool by APY, TVL and risk profile.</li><li>Enter an amount, review the estimate, then confirm the deposit.</li></ol></div><div class="docs-section"><h3>Testnet notes</h3><p>HOOD is a faucet token for testing only. Faucet claims have a cooldown; testnet balances have no cash value.</p></div><button class="primary full" @click="showDocs=false">Explore pools <b>↗</b></button></div></div>
